...
モノレポを作るには `${ROOT}/package.json` を作り、 `workspaces` キーを宣言する必要がある。テキストエディタで手書きすればいい。
